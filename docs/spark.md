...
원본 데이터는 약 40KB짜리 파일이 5만 개로 분산 저장된 상태였습니다. S3에서 파일을 읽을 때 파일 하나당 HTTP 요청이 한 번씩 발생하므로, 파일 수가 많을수록 네트워크 오버헤드가 선형적으로 증가합니다. 그 결과 단순 읽기에만 약 40분이 소요되었고, 이는 전체 런타임의 99%에 해당하는 극심한 병목이었습니다.
