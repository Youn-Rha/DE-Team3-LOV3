...
- **Step 3 : Impact Score 계산**
    ...
    df_with_score = df_normalized.withColumn(
        "impact_score",
        (F.abs(F.col("nor_accel_z")) * w1) + (F.abs(F.col("nor_gyro_y")) * w2)
    )

    # 3. 임계값 필터링 및 최종 반환
    # 특정 임계값을 넘는 행만 유지하여 Stage 2의 부하를 줄임
    # threshold = 3
    stage1_output = df_with_score.filter(F.col("impact_score") > threshold)
    ```
