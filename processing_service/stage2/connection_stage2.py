"""
Stage 2: 설정 및 Spark 연결 관리 모듈

이 모듈은 Stage 2 Spatial Clustering 및 Aggregation 파이프라인의 초기화를 담당합니다.
...
Example:
    >>> config = load_config("config_stage2.yaml")
...
def get_spark_session(config: Dict[str, Any]) -> SparkSession:
    ...
    app_name = spark_cfg.get("app_name", "Stage2-SpatialClustering")
    ...
    # Optional: Stage 2 specific configs if present in spark section
    join_broadcast_limit = spark_cfg.get("join_broadcast_limit")
    if join_broadcast_limit:
        builder = builder.config("spark.sql.autoBroadcastJoinThreshold", f"{join_broadcast_limit * 1024 * 1024}")
